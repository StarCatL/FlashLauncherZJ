...
public class CheatCodeExecutorControl extends Sprite {
    ...
    private static const SOL_NAME:String = "com.cheat.CheatCodeExecutorControl";
    ...
    private function tickStmtFrame(fr:Object):Boolean {
        ...
        switch (s.type) {
            ...
            case "If":
                ...
                _frames.push({
                    type: "StmtFrame",
                    node: truthy(tv) ? fr.cons : (fr.alt ? fr.alt : {type: "Block", body: []}),
                    state: 0,
    ...
    private function hasDefinitionDeep(domain:ApplicationDomain, name:String):Boolean {
        ...
        while (d) {
            if (d.hasDefinition(name)) return true;
            d = d.parentDomain;
        }
        return false;
    }
    ...
    private function getDefinitionDeep(domain:ApplicationDomain, name:String):* {
        var d:ApplicationDomain = domain;
        while (d) {
            if (d.hasDefinition(name)) return d.getDefinition(name);
            d = d.parentDomain;
        }
        return null;
    }
        ...
        switch (e.type) {
            ...
            case "Member":
                ...
                try {
                    ...
                    // fallback：用反射字符串
                    pushValue(evalExpression2(buildChainString(e), true));
                }
    ...
    private function envGet(name:String):* {
        for (var i:int = _envStack.length - 1; i >= 0; i--) {
    ...
    private function isUnboundRootName(name:String):Boolean {
        // builtin 名字不算
        if (_builtinNames.hasOwnProperty(String(name).toLowerCase())) return false;

        // import alias 不算
        if (_importMap.hasOwnProperty(name)) return false;

        // 当前 env 里有变量/函数也不算
        var v:* = envGet(name);
        if (v !== undefined) return false;

        // 像 true/false/null 这种不会走到这，但保险
        if (name == "true" || name == "false" || name == "null") return false;

        // 只有合法标识符才可能是“包链根”
        if (!isValidIdent(name)) return false;

        return true;
    }

    private function isValidIdent(s:String):Boolean {
        // 按你 tokenizer 的规则：这里给个简化版
    ...
    private function evalName(name:String):* {
        ...
        // 未绑定根名：一律当成“待解析链”根（pkgChain）
        if (isUnboundRootName(name)) {
            return makePkgChain({type: "Name", value: name});
        }

        // 否则才尝试反射（给那些确实是全局函数/对象的情况兜底）
    ...
    private function execAssign(left:Object, rv:*):* {
        ...
        if (left.type == "Member") {
            ...
            try {
                ...
                evalExpression2(buildChainString(left.object) + "." + left.prop + "=" + literalToString(rv), true);
            }
    ...
    private function resolveCalleeValue(node:Object):Object {
        ...
        if (node.type == "Member") {
            ...
            try {
                ...
                return {
                    fn: function (...args):* {
                        var callExpr:String = buildChainString(node.object) + "." + node.prop + "(" + args.join(",") + ")";
                        return evalExpression2(callExpr, true);
                    }, thisObj: null
                };
            }
